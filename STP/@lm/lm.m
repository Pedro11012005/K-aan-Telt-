...
[p, q] = size(a); % a partir de aquí se hizo una modificación
if issparse(a) % verifica si a es sparse y si si obtiene los valores diferentes de 0 para luego evaluar si es una matriz booleana con el if
    vals = nonzeros(a);
    if all(vals == 0 | vals == 1)
        col_sums = sum(a, 1); % suma por columnas, se mantiene sparse
        if all(col_sums)

        [i,~] = find(a == 1);  % Solo usamos los índices de fila
        k = mod(i', p);
        m.v = k + (k == 0) * p;

            m.n = p;
        else
            error('Each row must have at least one 1.');
        end
    else
        error('Sparse matrix must contain only 0 and 1.');
    end
else
