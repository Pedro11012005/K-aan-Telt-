...
for i = find(f)
    matches = strmatch(vars{i}, varstr, 'exact');
    if isempty(matches)
        error('Variable "%s" not found in varstr.', vars{i});
    elseif length(matches) > 1
        warning('Multiple matches found for "%s". Using the first match.', vars{i});
        f(i) = matches(1);
    else
        f(i) = matches;
    end
...
lm = sprintf('double(%s)', lm); % Lo que se cambia es que ahora la cadena de texto generada en la 150 se cambia a sparse con double
